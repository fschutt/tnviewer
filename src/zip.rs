...
fn strip_prefix(p: &str, top_folder_name: &str) -> Option<String> {
    if p == top_folder_name {
        ...
        Some(
            p.strip_prefix(&format!("{top_folder_name}/"))
                .map(|s| s.to_string())
...
pub fn read_files_from_zip(
    zip: &[u8],
    remove_top_folder: bool,
    dotfiles: &[&str],
) -> Vec<(Option<String>, PathBuf, Vec<u8>)> {
    ...
    for i in 0..zip.len() {
        ...
        let filename = file_n
            ...
            .and_then(|s| {
                if s.starts_with(".") && !dotfiles.iter().any(|q| *q == s) {
                    None
                } else {
                    Some(s)
                }
        ...
        let parent_dir = file_n
            .parent()
            .map(|s| s.as_os_str().to_string_lossy().to_string());
        let parent_dir = if parent_dir.clone().unwrap_or_default().is_empty() {
            None
        } else {
            parent_dir
        };
    ...
    let top_folder_name = match contents
        .iter()
        .find(|s| s.0.clone().unwrap_or_default().is_empty())
        .map(|s| s.1.display().to_string())
    {
        Some(s) => s,
    ...
    if remove_top_folder {
        contents = contents
            .into_iter()
            .map(|(parent, filename, contents)| {
                (
                    parent
                        .as_ref()
                        .and_then(|s| strip_prefix(s, &top_folder_name)),
                    filename,
                    contents,
                )
            })
