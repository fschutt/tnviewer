...
fn get_alle_flst_internal(datensaetze: &CsvDataType) -> Vec<FlstIdParsedNumber> {
    ...
    for (ds, _v) in datensaetze.get_old_fallback().iter() {
...
impl FlstIdParsedNumber {
    ...
    pub fn format_start_str(&self) -> String {
        ...
        let n = if let Some(n) = flst_nenner {
            format!("{n:05}")
        } else {
            String::new()
        };
...
impl FlstIdParsed {
    pub fn from_str(id: &str) -> FlstIdParsed {
        ...
        match gemarkung.parse::<usize>() {
            Ok(s) => {
                gemarkung = format!("{s:04}");
            }
            Err(_) => {
                gemarkung = "0000".to_string();
            }
        }
        ...
        match flst_zaehler.parse::<usize>() {
            Ok(s) => {
                flst_zaehler = format!("{s:05}");
            }
            Err(_) => {
                flst_zaehler = "00001".to_string();
            }
        }
    ...
    pub fn parse_num(&self) -> Option<FlstIdParsedNumber> {
        Some(FlstIdParsedNumber {
            ...
            flst_nenner: self.flst_nenner.trim().parse::<usize>().ok().and_then(|s| {
                if s == 0 {
                    None
                } else {
                    Some(s)
